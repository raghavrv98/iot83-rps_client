...
export class ManageRolePermission extends React.Component {
  state = {
    isLoading: true,
    isOpen: false,
    permissions: false,
  ...
  componentWillReceiveProps(nextprops) {
    if (
      nextprops.submitSuccess &&
      nextprops.submitSuccess !== this.props.submitSuccess
    ) {
      this.setState({
        isLoading: false,
        isOpen: true,
        message: this.props.match.params.id ? (
          <FormattedMessage {...messages.updateMessageHandler} />
        ) : (
            <FormattedMessage {...messages.submitSuccessMessage} />
          ),
        type: "success",
    ...
    if (
      nextprops.roleDetails &&
      nextprops.roleDetails !== this.props.roleDetails
    ) {
      let entitlements = JSON.parse(JSON.stringify(this.state.entitlements));
      if (
        nextprops.roleDetails.roleEntitlementDTOList.filter(
          val => val.resource !== "PROFILE"
        ).length > 0
      ) {
        if (
          nextprops.roleDetails.roleEntitlementDTOList.some(
            item => item.resource === "ALL"
          )
        ) {
          ...
          nextprops.roleDetails.roleEntitlementDTOList.map(val => {
            entitlements.map(item => {
              item.minPermissions =
                val.minimumPermissions[0] == "ALL"
                  ? ["READ", "CREATE", "UPDATE", "DELETE", "ALL"]
                  : val.minimumPermissions || [];
            });
          });
          this.setState({
            entitlements,
          ...
          nextprops.roleDetails.roleEntitlementDTOList.map(val => {
            if (val.resource !== "PROFILE") {
              entitlements.filter(
                item => item.resource === val.resource
              )[0].permissions =
                val.permissions[0] == "ALL"
                  ? ["READ", "CREATE", "UPDATE", "DELETE", "ALL"]
                  : val.permissions;
          ...
          nextprops.roleDetails.roleEntitlement =
            nextprops.roleDetails.roleEntitlementDTOList;
          this.setState({
            permissions: true,
            entitlements
          });
    ...
    if (
      nextprops.getPermissionsDetails &&
      nextprops.getPermissionsDetails !== this.props.getPermissionsDetails
    ) {
      let permission = {};
    ...
    if (
      nextprops.getEntitlementsSuccess &&
      nextprops.getEntitlementsSuccess !== this.props.getEntitlementsSuccess
    ) {
      let entitlements = [];
      ...
      this.setState(
        ...
        () => {
          this.props.match.params.id &&
            this.props.getRoleDetails(this.props.match.params.id);
        }
      );
    }
  ...
  nameChangeHandler = (event, index) => {
    ...
    if (event.target.checked)
      ...
      entitlements[index].permissions.splice(
        entitlements[index].permissions.indexOf(event.target.value),
        entitlements[index].permissions.length
      );
    this.setState({
      entitlements,
      allPermission: []
  ...
  allPermission = event => {
    ...
    this.setState({
      entitlements,
      allPermission: event.target.checked
        ? [{ resource: "ALL", permissions: ["ALL"] }]
        : []
    });
  };
  ...
  render() {
    return (
      <div className="appContent">
        <Helmet>
          <title>ManageRolePermission</title>
          <meta name="description" content="Description of ManageRolePermission" />
        </Helmet>
        ...
        {this.state.isLoading ?
          <SkeletonLoader  skeleton="skeletonPermissions" mode="fullView"/>
          :
          <React.Fragment>
            {this.state.permissions && (
              <div className="contentForm">
                <h5 className="formHeader">
                  <FormattedMessage {...commonMessage.assignPermissions} children={(message => message)} />
                  <div className="headerItemRight">
                    <div className="switchButtonLabel">
                      ...
                      <label
                        className={
                          ["ACCOUNT_ADMIN", "SYSTEM_ADMIN"].includes(
                      ...
                      >
                        <input
                          type="checkbox"
                          disabled={["ACCOUNT_ADMIN", "SYSTEM_ADMIN"].includes(
                            this.state.payload.name
                          )}
                          onChange={this.allPermission}
                          id="ALL"
                          checked={this.state.allPermission.length > 0}
                        />
                ...
                <table className="table customHTMLTable permissionTable">
                  <thead>
                    <tr>
                      <th width="25%">
                        <FormattedMessage {...commonMessage.entitlements} children={(message => message)} />
                      </th>
                      <th width="15%">
                        <FormattedMessage {...commonMessage.read} children={(message => message)} />
                      </th>
                      <th width="15%">
                        <FormattedMessage {...commonMessage.write} children={(message => message)} />
                      </th>
                      <th width="15%">
                        <FormattedMessage {...commonMessage.update} children={(message => message)} />
                      </th>
                      <th width="15%">
                        <FormattedMessage {...commonMessage.delete} children={(message => message)} />
                      </th>
                      <th width="15%">
                        <FormattedMessage {...commonMessage.all} children={(message => message)} />
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                            ...
                            <input
                              ...
                              disabled={
                                ["ACCOUNT_ADMIN", "SYSTEM_ADMIN"].includes(
                                  ...
                                  ? true
                                  : (val.minPermissions || []).includes("READ")
                                    ? true
                                    : false
                              }
                              type="checkbox"
                            ...
                            <input
                              ...
                              disabled={
                                ["ACCOUNT_ADMIN", "SYSTEM_ADMIN"].includes(
                                  ...
                                  ? true
                                  : (val.minPermissions || []).includes("ALL")
                                    ? true
                                    : false
                              }
...
const withReducer = injectReducer({ key: "ManageRolePermission", reducer });
const withSaga = injectSaga({ key: "ManageRolePermission", saga });
