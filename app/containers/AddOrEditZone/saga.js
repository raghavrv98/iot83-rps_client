...
export function* addOrEditZoneSaga(action) {
  ...
  if(action.isBulkUpload)
    url = window.API_URL + "faas/api/v1/plants/" + action.plantId + "/pipelines/" + action.pipelineId + "/bulk/ehtZones"
  if (action.zoneId) 
...
export function* apiGetZoneDetailsHandlerAsync(action) {
  try {
    ...
    url = url + "/" + action.plantId + "/pipelines/" + action.pipelineId + "/ehtZones/"+ action.zoneId;
